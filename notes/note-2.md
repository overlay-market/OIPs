...
If we were to naively choose the last fetched oracle value \\( P_i \\) to use as this position's entry price, the system could be easily exploited. To see how this would work, make this naive assumption and take \\( P(t) = P_i \\). Users have more up to date information about the current spot price (and thus the TWAP) than the Overlay market does at time \\( t \\).
